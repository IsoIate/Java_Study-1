...
public class Singleton {
 ...
 ## 5. 템플릿 메서드 패턴(Template Method Pattern)  
  ...
  - 동일한 기능에 부가적인 부분이 다른, 예를들면 다음과 같은 상황일 경우  
  ```java
  public Class Dog{
    public void playWithOwner(){
        System.out.println("귀염둥이 이리온 ~~~");
        System.out.println("멍! 멍!"); // 해당 라인 제외 Cat 클래스와 동일
  ...
  public Class Cat{
    public void playWithOwner(){
        System.out.println("귀염둥이 이리온 ~~~");
        System.out.println("야옹! 야옹!"); // 해당 라인 제외 Dog 클래스와 동일
        System.out.println("꼬리 살랑 살랑");
        System.out.println("잘했어");
    }
  }
  ...
  - 다음과 같이 메소드를 분리하여 달라지는 부분을 메서드로 추출 하여 Hook(갈고리) 메서드를 만든다
  ```java
  ...
  ```
  - 템플릿 메서드는 상위 클래스에 공통 로직을 수행하는 템플릿 메서드와 하위 클래스에 오버라이딩을 강제하는 추상 메서드 또는 선택적으로 오버라이딩 할 수있는 훅(Hook) 메서드를 두는 패턴이다.  
